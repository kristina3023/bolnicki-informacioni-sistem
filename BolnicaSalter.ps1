...
$btnZakazi.Add_Click({
    if ($cbPacijent.SelectedIndex -ge 0 -and $cbDoktor.SelectedIndex -ge 0) {
        ...
        $pacijentId = [regex]::Match($pTekst, '\[(\d+)\]').Groups[1].Value
        $doktorId   = [regex]::Match($dTekst, '\[(\d+)\]').Groups[1].Value
        $datumVreme = $dtp.Value.ToString("yyyy-MM-dd HH:mm:00")

        $sql = "INSERT INTO pregled (pacijent_id, doktor_id, datum_vreme, status) VALUES ($pacijentId, $doktorId, '$datumVreme', 'Zakazan')"
...
$btnDodajPacijenta.Add_Click({
    ...
    if ($ime -eq "" -or $prezime -eq "" -or $jmbg.Length -ne 13 -or $lbo.Length -ne 11 -or $polisa -eq "") {
        [System.Windows.Forms.MessageBox]::Show("Unesite ispravno ime, prezime, JMBG (13 cifara), LBO (11 cifara) i broj polise!", "Upozorenje", "OK", "Warning")
        return
    }

    $sqlPac = @"
        INSERT INTO pacijenti (ime, prezime, jmbg, lbo, datum_rodjenja) 
        VALUES ('$ime', '$prezime', '$jmbg', '$lbo', '$datum');
        
        DECLARE @novi_id INT = SCOPE_IDENTITY();

        INSERT INTO osiguranje (broj_polise, pacijent_id, naziv_fonda, procenat_pokrica, godisnji_max)
        VALUES ('$polisa', @novi_id, '$fond', $pokrice, NULL);
"@
